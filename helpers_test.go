...
// filterNonLinuxZfsContent strips out content generated by 15_linux_zfs
func filterNonLinuxZfsContent(t *testing.T, path, outPath string) {
	// We strip only the relevant content from 15_linux_ZFS
	f, err := os.Create(outPath)
	if err != nil {
		t.Fatal("couldn't create grub_15_linux_zfs file", err)
	}
	defer f.Close()
	...
	for s.Scan() {
		t := s.Text()
		if strings.HasPrefix(t, "### END") && strings.Contains(t, "/15_linux_zfs") {
			shouldWrite = false
		}
		if shouldWrite {
		...
		if strings.HasPrefix(t, "### BEGIN") && strings.Contains(t, "/15_linux_zfs") {
			shouldWrite = true
		}
	}
